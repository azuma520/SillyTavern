...
| `RP記憶/實驗與驗證/LearnedSelf_MVP設計.md` | **Learned Self 的設計權威**：八條核心原則、三層資料單位（Experience／Meaning／Scope）、LS-001 定稿、§十一 ownership binding（世界書表達不了「這份 state 屬於誰」） | 開發 Learned Self 的任何一步 |
| `RP記憶/實驗與驗證/Diary_Audit_能否當LearnedSelf上游_2026-09-05.md` | 日記管線 audit：逐字 prompt、`diaryCharLimit=2` 等實際設定、50 篇欄位覆蓋率、**四個缺陷**（複製污染／窄視窗／`secret` 會編造／`mood` 幾乎無資訊） | **改 `character-diary` 之前**，或要把日記當任何下游的資料源時 |
| `RP記憶/實驗與驗證/LS_MechanismTest_第二輪_三層證據總結_2026-09-05.md` | 機制測試線結案：三層合成、Gate 2 中段不得宣稱成立、§七 收束為使用者判斷而非閘門觸發 | 要引用「注入有沒有效」的結論時（**別引原始材料**） |
...
> 2026-09-06 併入兩條同源規則（**Instrument** / **Occurrence**，各 5 件可稽核 case）。
> 這是**規範層**；機械 enforcement（hook）刻意延後，見下方「為什麼現在沒有 hook」。
>
> **證據鏈**：三批的原始 case 全部住在 `文檔/handoff/session-handoff-20260829.md`、
> `-20260830.md`、`-20260905.md` 的「四、洞見 / 反省」；backlog 原行升級後依 `[graduated:]`
> 規則刪除，逐條推導過程查 `git log -p -- backlog.md`。**引用件數前先跑 evidence audit**。
...
- **Discrimination**——判準能區分預期差異，避免地板 / 天花板效應或純主觀判讀。
  **並且**：用這把尺、**這個樣本量**，理論上有沒有能力判出宣稱要判的差異。
  （2026-08-30 補。**改變比較基準 = 改變整個統計問題，n 必須跟著重算**——
  A/D positive control 把對照從「已知常數基線 86.7%」改成「同場現跑的 5 則」，
  修正本身是對的，但兩邊都有抽樣誤差之後 n=5 的鑑別力大幅下降，
  而這在收資料前就算得出來。白話：原本拿刻度已知的尺量東西，
  改成拿兩根自己也有誤差的棍子互相比。）
- **Construct**——測量結果確實對應要驗證的問題，而不是代理變數或另一個構念。

**量測工具與執行當下另確認兩項**（2026-09-06 併入）：

- **Instrument**——量測工具寫完先用**已知答案**校準。校準不符時修工具的缺陷、**不是**把參數調到吻合；
  校準完就**凍結**（hash / commit），不得在看到新資料後再改。
  （踩過：驗證腳本有一項寫成 `... or True`、恆真永遠 PASS；三層對齊腳本把「不計票」的票算進某一家，
  該家顯示 4/18、實為 0/18。兩次都是「逐家計數須等於 X」這種**對帳**擋下的，不是看出來的。）
- **Occurrence**——驗證某訊號「有沒有出現」之前，先確認**會產生該訊號的事件真的發生過**，
  否則 log 上的有與無都只代表什麼都沒跑。做法：**在 UI／工具回報之外找一個不會騙人的計數器**
  （查聊天檔 swipes 數，而不是更仔細讀 console；`exit code 0` 不代表真的跑過）。
  四個層次都漏過：執行層（事件沒跑就讀 log）、回傳碼層、搜尋層（沒找遍就宣告不存在）、
  **構念層**（觀察條件下 X 從未被觸發，卻把「沒觀察到 X」讀成「沒有 X 的傾向」）。
  **代價不對稱**：前三層漏接賠幾則資料，構念層那次賠掉一整輪實驗設計。

**三者分工**：Observation 問「觀察點存不存在」（設計時）、Occurrence 問「觀察的那一刻該發生的事
發生了沒」（執行時）、Instrument 問「儀器準不準、以及誰有權在什麼時候改它」。
...
### backlog 條目升級五問（2026-08-30 由使用者律定）

一件事**不會**因為發生過就自動變成可復用的規則。順序走五道，**任一道不過就停在該層**：

```
① 證據是真的嗎？（Evidence Audit）
② 值得記嗎？
③ 可重複嗎？
④ 能行動嗎？
⑤ 有邊界嗎？
...
**① 證據是真的嗎——升級前的前置，不是最後才查。**
「這條說有 5 個 case」→ 那 5 個真的存在嗎？真的是同一種問題嗎？原始出處找得到嗎？
衍生狀態（`[case-count:]` / `[mature:]`）**不可**直接當證據。
來源：2026-08-30 查出某條 `[case-count: 5]` 實際只有 2 件可稽核證據。

**② 值得記嗎——四個面向**（白話：這件事值得我以後特別記得嗎）
- **影響大不大**：下次再犯 / 再遇到，會不會明顯改變結果？
- **有沒有證據且可能重複**：一次偶發，還是已看出規律？
- **風險與可恢復性**：做錯好不好救？**難救的即使頻率低也值得記**
- **改善成本與槓桿**：很小的成本記住 / 改掉，能不能省下未來很多成本？

**③ 可重複嗎**——能不能套到不只這一次。
❌「這次 uid 21 出了問題」→ ✅「驗證一個訊號不存在之前，先確認產生訊號的事件真的發生過」

**④ 能行動嗎**——能不能直接告訴下次怎麼做。
❌「上次搞錯了，好慘」→ ✅「看到 0 筆 log 之前，先驗證觸發事件確實執行」

**⑤ 有邊界嗎**——什麼時候不適用，避免有用的經驗變成萬用教條。
例：「正式驗證要跑 Preflight」的邊界是「低成本、可逆、純探索性的 smoke test 不必走完整 Gate」。

#### 授權範圍

**agent 自行判斷、不逐條報告**：開不開新條目、bump 哪一條、tag 選哪個、要不要合併同一 episode。
**只有「要升級成正式載體」時才報告使用者**（寫進 CLAUDE.md / hook / Skill / spec 等）——
那才是需要拍板的那一步。
