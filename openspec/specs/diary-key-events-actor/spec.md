# diary-key-events-actor

## Purpose

讓 `character-diary` 日記的 `key_events` 每條都帶有自足的行為主體，使「誰做了什麼」不再靠讀者依日記主人補上。這是 Learned Self 讀取日記時的輸入契約：`key_events` 是六個欄位裡唯一屬於 Experience 核心的一格，主體一旦錯置，整條推論都偏。

本能力的範圍限於 **prompt 契約**：`key_events` 維持字串陣列，主體以「主体：事件」前綴表達；不改 schema、不遷移舊資料、不動任何讀取點。
