# diary-scene-player-messages

## Purpose

讓 `character-diary` 擴充的日記 prompt 在「本次劇情片段」中納入玩家的原始發言，而不是只餵 AI 樓層。

在此能力之前，日記模型看得到角色說了什麼、做了什麼，卻看不到**玩家在前面說了什麼、做了什麼**。缺的不是文字量，而是「角色為什麼會這樣反應」的因果上下文——同一句角色反應，在看不到玩家原話時，模型可能把角色的心理方向理解反。

本能力的範圍限於**日記 prompt 的組裝**：玩家樓層只在組 prompt 時從聊天陣列讀取，不進入任何進度游標，關係與劇情檔案兩路不受影響。
