# diary-auto-summary-toggle

## Purpose

讓 `character-diary` 擴充的自動總結開關 `autoSummary` 可以在設定面板操作，而不是只能經瀏覽器主控台切換。

旗標本身早已存在於 `DEFAULT_SETTINGS`（預設 `true`）並被 `cdOnMessageReceived` 與 `cdCheckAutoTrigger` 尊重；缺的只是面板接線與狀態可見性。這個能力的用途是**觀察模式**——做對照實驗或凍結主線時，需要能可靠地停掉自動寫日記，並且看得出目前是停著的。
