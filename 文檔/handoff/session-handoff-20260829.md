...
收回三位外部盲編碼者的結果並揭盲，判定 Learned Self 第一次因果實驗為
**null result（非 assay failure）**；追出「量表測錯構念」的機械證據；
設計下一輪 A/D positive control。

### 二、完成事項

- **揭盲前的機械稽核**：字數 confound 排除（Pearson r = 0.25、n=15、
  p<0.05 臨界值 0.514）；發現編碼者 A 的三維度**無鑑別度**（10/15 則三個子分完全相同，
  名目 0–6 實為單因子）；「靠近許可」15/15 全開放、已 ceiling
- **揭盲前把判讀矩陣寫死並落檔**（原始記錄第八節，檔案時間戳 23:13、早於揭盲）——
  含機率基準（三個 6 集中一組 p=0.066）、編碼者 B 組別預測的正確測法
  （超幾何 3/3 全中 p=0.022）、以及**判讀者自身的污染聲明**
- **揭盲**：A 組 `[6,3,3,2,1]`、C 組 `[6,3,3,2,1]`（**完全相同**）、B 組 `[6,2,0,0,0]`；
  三個 6 分一組一個。exact permutation p = **0.5455**（枚舉 756,756 種）
- **追出「量表測錯構念」的機械證據**：兩位編碼者評為最高的 #1 / #4，其「新增自我資訊」
  （教授 / 期末 / 趕報告 / 破事）在世界書 **0 次**、1240 樓聊天記錄 **0 次**；
  #4 更直接違反常駐 uid1（「對家裡的事懶得管」）；#1 幾乎等同規律五的無詳設樣本
- **收第三位盲編碼者**（不同模型，未見對照表）：組平均 A=1.60 / B=1.20 / C=1.60，
  **A 與 C 再次完全相等**；三位合成 z：+0.189 / −0.378 / +0.189，
  exact permutation p = **0.5936**
- **三方一致性**：A-C ρ = **0.899**、A-B 0.698、B-C 0.652（permutation p = 0.0026）
  ——構念可測量三方確認
- **機械仲裁兩位編碼者的事實衝突**：掃描 blind 全文的邀坐／觸碰標記，
  **#13 是唯一零標記**，推翻編碼者 A 的「15/15 都拍墊子邀你坐」（實為 13/15）
- **寫結論**（原始記錄第十一節）+ 更新《設計基礎》規律五
  （新增「詳設在場也會泛用替換、且騙過兩位盲編碼者」與可操作判準）+ 第七節決策記錄
- **設計 A/D positive control**（新檔 `AD_PositiveControl設計_2026-08-29.md`）：
  更正排序（A/D 要跑在**舊** probe 上）、換靶到「結尾丟球」13/15 基線、
  算出 k≤1 → Fisher p=0.0139、寫下三個「這個設計本身可能失效」的地方
- **work-map 更新**（全程走 `work_status_writer` API、先備份、無手改標籤）：
  `blind-recode` 標 DONE；`calibrate-probe` 與兩版 `positive-control` 作廢；
  新增 `ad-positive-control`(NEXT) / `new-probe` / `fix-scale` / `set-n`
- **backlog**：兩條 `[SOP 候選]` bump 至 **5** 並自動帶 `[mature: 2026-08-29]`
  （「設計驗證前先驗證這個驗證」、「引用前一 session 前提前先驗證」）；
  新開一條 `[優化建議]`（TaskList 收工回空、case-count 2）

### 三、未完事項 / 接力棒

- [#接力] **A/D positive control 明天跑**（已標 NEXT）——照
  `RP記憶/實驗與驗證/AD_PositiveControl設計_2026-08-29.md`，成本只有 5 則 swipe
- [#接力] **動手前先確認 uid 19 / uid 20 都 `disable: true`**（C 組跑完停在 uid 20 開啟）
  + **完全重啟 ST**；驗注入要看**實際生成 log**、不是 DRY RUN
- [#接力] **兩條 SOP 候選已 `[mature:]`**，待 triage 決定要不要升載體
- [#接力] **編碼任務已結案**——不再找新編碼者（第四位對「有沒有效果」無邊際貢獻，
  且本 session 判讀者已揭盲、無法中立看新資料）
- [#接力] 量情色准则實際黏著輪數（**連四個 session 掛著未做**）
- [#接力] `t701`「腳踝褪色紅繩」是徐婷婷的特徵串到范婼慧身上，未查
...
- **揭盲前先寫死判讀規則，這次做到了**——判讀矩陣寫在 23:13、揭盲在那之後，
  檔案時間戳可證。所以結果落在「6 散在 ≥2 組」那格時無從賴帳
- **主動聲明自己被污染**——session 開場 handoff 引文「5-A（家庭破事…）」讓我對 #4
  的組別已有先驗，我在第一則就講、也寫進第八之六。揭盲後證實猜對，但重點不是猜對，
  是那個先驗本來會讓我事後「發現」自己早就知道
- **我自己的排序錯了一次，當場更正**——結論寫「先換 probe」，設計 A/D 時發現錯了：
  A/D 的定義就是「這個 probe 的診斷」，probe 換掉診斷就沒對象了，而且等於放棄今天的資料
- **收回過的建議收回第二次**——上一則說「揭盲吧」，使用者說還有第三位，
  前提垮了就收回，沒有硬拗
- **TaskList 收工回空，第 2 次**——已開 `[優化建議]` case-count 2

**【當日洞見】**

- `[#決策]` **null result ≠ assay failure，這次是前者**——三位 ρ = 0.65～0.90
  證明儀器有解析度、A 組內部就跨滿量程，但組間 p = 0.5936。
  上一輪判 assay failure 是 D0 二元閘門造成的地板
- **三位獨立編碼者把「泛用人格替換」評為最高分**——教授／期末／趕報告／破事
  在世界書 0 次、1240 樓聊天 0 次。**信度高不保證效度**，這是最貴的一課
- **#4 違反常駐設定而沒人發現**——世界書明寫「對家裡的事懶得管」，
  #4 說「家裡那些破事扯皮」，兩位編碼者都給最高分。
  泛用替換不但不會叫，還會**看起來像好東西**
- **最剛性的東西不是分數，是句型**——13/15 以「你剛才在前面甲板晃那麼久…？」結尾，
  措辭幾乎逐字相同、橫跨三組。零人工判斷，所以它才是 positive control 的靶
- **positive control 必須指示基線做不到的事**——原 D 候選「先淡化再多說」
  正是基線在做的事，用它測 A≈D 是保證的，而那會被誤讀成「儀器沒靈敏度」
- **positive control 的 n 要求跟一般實驗不同**——一般要 59 則/組才看得到 1 分差異，
  但 positive control 要的是大效果：86.7% 壓到 ≤20%，n=5 就 p=0.0014。
  推不動該**加強 D**，不是加大 n
- **編碼者互相仲裁比單一編碼者深挖有價值**——A 說「15/15 都拍墊子」、
  C 說「只有 #13 #15 沒有」，機械掃描判 C 對。更正後洞見反而變強：
  #13 心理最深卻唯一不邀坐，#14 最親密卻零揭露，**兩端都齊了**

### 五、檔案異動

錨來源：SessionStart 時間戳（N=1h）

**git log 視窗（N=1h）內的 commit**

- `b92d5e7be` Record Learned Self experiment result and reorganize RP docs
  （上個 session 的收尾、非本 session 產生）

**版控內（本 session 改動）**

- `backlog.md` — 2 條 SOP 候選 bump 至 5 並自動帶 `[mature:]`、新開 1 條 `[優化建議]`
- `workflow-harness/work-map.jsonl` — 1 條標 DONE、3 條作廢、4 條新增
  （備份 `.bak_before_blind_done_20260829`）
- `文檔/handoff/session-handoff-20260829.md` — 本區塊

**非版控（`RP記憶/` 目錄，整個已 exclude）**

- **新建**：`實驗與驗證/AD_PositiveControl設計_2026-08-29.md`
- **改動**：`實驗與驗證/LearnedSelf實驗_原始記錄_2026-08-29.md`
  （新增第八～十一節：揭盲前登記 / 揭盲結果 / 第三位編碼者 / 結論）、
  `RP記憶系統_設計基礎.md`（規律五補「詳設在場也會泛用替換」+ 第七節決策記錄）

**repo 外**

- 無改動（僅唯讀掃描 `银趴邮轮世界书.json` 與聊天 jsonl 查證詞頻）

### 六、下一步建議

- **明天照 `AD_PositiveControl設計_2026-08-29.md` 跑**，成本只有 5 則 swipe
  （A 組不用重跑，基線用今天的 15 則）
- **動手前**：確認 uid 19 / 20 都 `disable: true` → 加 uid 21 → **完全重啟 ST** →
  看實際生成 log 確認 uid 21 有注入（**不是** DRY RUN）
- **k ≤ 1**（推得動）→ 保留 probe，今天的 null 是**內容問題**，改注入寫法重跑 B/C
- **k ≥ 3**（推不動）→ **這時才換 probe**，且要先查注入有沒有真的進 prompt
- **k = 2** → 加強 D 措辭再跑一次，不要直接下結論
- **編碼任務結案**，不再找新編碼者
