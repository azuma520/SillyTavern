...
- [優化建議] [case-count: 6] [mature: 2026-09-05] `/end-session` 第一步的 `TaskList` 撈不到本 session 的 completed task（回 "No tasks found"），完成事項每次都得從 context 重建——Guardrail A4 要求跑 TaskCreate，但收工端接不到它的產物
...
- [SOP 候選] [case-count: 4] 量測工具寫完要先用已知答案校準；校準不符時修工具的缺陷、不是把參數調到吻合，且校準完就凍結（hash / commit）不得在看到新資料後再改
