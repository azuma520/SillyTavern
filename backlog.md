...
- [優化建議] [case-count: 1] 驗收設計裡每一個「不該發生」的格子，凍結前要用舊碼基線**逐格**實測它不是地板——CLAUDE.md Discrimination 寫了「避免地板效應」，但地板是設計時推想不出來的：這輪 7 個不在場格只有 1 格在舊碼會誤寫，其餘 6 格的正式 0 只證明無退化，E 情境的保守規則因此整個量不到；「合計 ≥ 1 就有效」的基線判準也擋不住這種局部地板。Discrimination 那條需要補「逐格、實測、凍結前」三個字眼
  → handoff 20260913 四（Step 3 presence gate：基線 12 run 後才發現 S1／S3／S4 的 D／E 格舊碼即為 0，只有 S2 徐一格有鑑別力；設計缺口記入 design Risks 與驗收檔 §2.3）
...
- [SOP 候選] [case-count: 1] 量測工具的凍結單位是可重跑的腳本本體（含名單、代名詞集合等所有參數），不是「規則描述＋結果數字」——只留規則與數字，下次要用同一把尺時得重建，重建出來的尺跟原尺對不上就分不清是資料變了還是尺變了
  → handoff 20260913 四（9/6 audit 的 key_events 分類器只留「前 6 字內有無角色名」與 52/5/19/48/124，腳本沒落檔；9/13 重建時名單與代名詞集合都要重猜，封閉集合內窮舉只能逼近到差 1 條，最後由使用者裁定接受殘差）
...
- [bug] [P3] character-diary 日記編輯器**儲存時用 `split(/[,，、]/)` 切 `key_events`，全形逗號也當分隔符**（`index.js` 約 7176 行，上游既有）：任何一條事件內含「，」的日記，只要在編輯器按「保存」（即使什麼都沒改），陣列就從 N 條裂成更多條。2026-09-13 Step 2 收尾驗標準 5 時實測：Branch #8 范婼慧 1249 樓新篇 3 條 → 5 條（「范婼慧：驚喜地追問簽證與課業，並暗示願意…」在「，」處被切成兩條、主詞前綴留在前半）。Step 2 spec 的 scenario「編輯器…儲存後陣列元素不變」對含「，」的資料不成立；actor 前綴格式本身沒問題，但事件描述常帶「，」，這個缺陷會把前綴和後半句拆散、產生無主詞的碎條。同次也查出標準 5 的另外兩個讀取點在本版 UI 不可達：時間軸（`cdRenderTimeline` 無任何呼叫點、`#cd-tb-timeline` 按鈕不存在於 HTML）、搜尋框（上游 2026-08-08 `style.css:1650` 以 `display:none !important` 隱藏）。標準 5 實際能驗的只有面板與編輯器
  → 修法候選：編輯器改用只認半形逗號或換行分隔、或儲存時不重切（欄位改多行 textarea 一行一條）。與日記品質線無關、屬擴充本身的 bug，另案處理；Branch #8 該篇的 5 條需人工還原成 3 條（原文在 handoff 20260913 與本 session 對話）
- [bug] [P3] [done: 2026-09-13] character-diary 日記會**替不在場的角色產生一篇「我沒出場」的日記**。2026-09-06 OFF／ON 對照實測（Branch #12、樓層 #1243–#1255）：OFF 態多產生一篇徐婷婷日記，`entry` 寫「這段時間我沒有出現在泳池派對中，也沒有參與……」、`key_events` 為空陣列、`secret` 還編了一句「希望自己之後仍有機會加入他們的行程」。同批 ON 態沒有這篇。花 token 生成、產出零資訊，且為該角色累積了一筆語意上不存在的記憶
...
- [SOP 候選] [case-count: 2] 把含反斜線或跳脫序列的內容寫進檔案時走檔案寫入工具、不走 shell heredoc——引號 heredoc 在本環境仍會改寫反斜線，錨點字串在傳輸中就變了形，比對失敗但看不出原因
